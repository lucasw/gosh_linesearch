...
// [[file:../linesearch.note::*original documentation by J. Nocera (lbfgs.f)][original documentation by J. Nocera (lbfgs.f):1]]

// original documentation by J. Nocera (lbfgs.f):1 ends here

// [[file:../linesearch.note::*entry][entry:1]]
...
mod mcsrch {
        ...
        Efn: FnMut(f64) -> Result<(f64, f64), LineSearchError>,
    {
        ...
        let mut width = vars.max_step - vars.min_step;
        ...
        for count in 0..max_iterations {
            ...
            *stp = stp.clamp(vars.min_step, vars.max_step);
            ...
            if f <= ftest1 && dg.abs() <= vars.gtol * -dginit {
                ...
                if 0 != stage1 && f <= ftest1 && vars.ftol.min(vars.gtol) * dginit <= dg {
                    stage1 = 0;
                }
...
// [[file:../linesearch.note::*mcstep][mcstep:1]]
...
// [[file:../linesearch.note::*interpolation][interpolation:1]]
